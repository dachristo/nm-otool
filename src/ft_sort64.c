...
int			ft_sort_alpha64(t_data *data, t_sort sort, t_options *options,
							int start1)
{
	char	*s1;
	char	*s2;
...
static int	ft_sort_same64(t_data *data, t_sort sort, int start1)
{
...
static void	ft_data64(t_data *data, int start1, int end1, int end2)
{
	t_sort	sort;
	int		i;
	...
	while (++i <= end2)
	{
		...
		else if (!ft_sort_alpha64(data, sort, data->ptr_file->options, start1))
		{
			if (ft_sort_same64(data, sort, start1))
